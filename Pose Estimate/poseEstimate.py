...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while cap.isOpened():
        ...
        try:
            ...
            if (detectHandSignal(Nose, Left_Thumb, Left_Index_Tip, Left_Index_Pip, Left_Middle, Left_Ring,
                                 Left_Pinky) and
                    detectHandSignal(Nose, Right_Thumb, Right_Index_Tip, Right_Index_Pip, Right_Middle, Right_Ring,
                                     Right_Pinky) and
                    detectArmRaise(Left_Elbow, Right_Elbow, Nose) and
                    checkFace
            ):
                t1=time.time()
                timer = int(t1-t2)
                if(timer<5):
                    ...
                    drawPose((0, 0, 255))
                    cv2.putText(image, "SENDING", (0, int(height / 2)), cv2.FONT_HERSHEY_DUPLEX, 3,
                                (0, 0, 255), 2)
                    cv2.putText(image, "ALERT", ( 0, int((height / 2)+(height/3)) ), cv2.FONT_HERSHEY_DUPLEX, 3,
                                (0, 0, 255), 2)
